...
import { LAST_ACTION_STORAGE_KEY, LAST_MOOD_EVENT, LAST_MOOD_STORAGE_KEY, LAST_MOOD_UPDATED_KEY, VALID_ACTIONS, isMoodKey } from "@/lib/mood";
...
export function AiChat() {
  ...
  const applyDetectedPreferences = useCallback((mood: string | null, action: string | null, movies: MovieDetails[] = []) => {
    ...
    const normalizedAction = mappedAction && VALID_ACTIONS.includes(mappedAction as (typeof VALID_ACTIONS)[number]) ? mappedAction : null;
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          messages: updatedMessages.filter(m => m.role !== "assistant" || m === INITIAL_MESSAGE || m.content !== INITIAL_MESSAGE.content).map(m => ({ role: m.role, content: m.content })),
        }),
      });
    ...
    <>
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 z-50 flex h-14 w-14 items-center justify-center rounded-full shadow-[0_4px_24px_rgba(0,0,0,0.5)] transition-all duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-400 ${
          isOpen ? "bg-white/10 backdrop-blur-md border border-white/15 text-white rotate-0" : "bg-amber-400 text-black hover:bg-amber-300 hover:scale-105"
        }`}
      ...
      {/* Chat panel */}
      <div
        className={`fixed bottom-24 right-6 z-50 w-90 max-w-[calc(100vw-3rem)] transition-all duration-400 ease-out ${isOpen ? "opacity-100 translate-y-0 pointer-events-auto" : "opacity-0 translate-y-4 pointer-events-none"}`}
        role="dialog"
        aria-label="AI Movie Recommendations Chat"
        aria-hidden={!isOpen}>
        <div
          className="flex flex-col rounded-2xl border border-white/12 bg-black/92 backdrop-blur-xl shadow-[0_8px_40px_rgba(0,0,0,0.7)] overflow-hidden"
          style={{ height: "520px" }}>
          {/* Header */}
                ...
                <div className={`max-w-[80%] ${message.role === "user" ? "items-end" : "items-start"} flex flex-col gap-2`}>
                  <div
                    className={`rounded-2xl px-3 py-2 text-sm leading-relaxed ${
                      message.role === "user" ? "bg-amber-400/20 border border-amber-400/25 text-white rounded-tr-sm" : "bg-white/6 border border-white/8 text-white/85 rounded-tl-sm"
                    }`}>
                    {message.content}
                  </div>
