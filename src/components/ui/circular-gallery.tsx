...
class Media {
  ...
  constructor({
    ...
    this.font = font;
    this.posterWidthPx = posterWidthPx;
    this.posterHeightPx = posterHeightPx;
    this.createShader();
    this.createMesh();
    this.createTitle();
    this.onResize();
    ...
    this.program = new Program(this.gl as any, {
      ...
      vertex: `
        ...
        void main() {
          vUv = uv;
          vec3 p = position;
          p.z = (sin(p.x * 4.0 + uTime) * 0.4 + cos(p.y * 2.0 + uTime) * 0.4) * (0.02 + uSpeed * 0.15);
  ...
  onResize({ screen, viewport }: { screen?: { width: number; height: number }; viewport?: { width: number; height: number } } = {}) {
    ...
    if (this.posterWidthPx && this.posterHeightPx) {
      this.plane.scale.x = (this.viewport.width * this.posterWidthPx) / this.screen.width;
      this.plane.scale.y = (this.viewport.height * this.posterHeightPx) / this.screen.height;
    } else {
      this.scale = this.screen.height / 1500;
      this.plane.scale.y = (this.viewport.height * (680 * this.scale)) / this.screen.height;
...
class App {
  ...
  constructor(
    ...
    this.onResize();
    this.createGeometry();
    this.createMedias(items, bend, textColor, borderRadius, font, posterWidthPx, posterHeightPx);
    this.update();
  ...
  onTouchDown(e: MouseEvent | TouchEvent) {
    ...
    this.start = (e as TouchEvent).touches
  ...
  update() {
    ...
    // Track which item is centered
    if (this.medias && this.medias[0] && this.onActiveIndexChange) {
      const width = this.medias[0].width;
      if (width > 0) {
        const originalLength = this.mediasImages.length / 2;
        const rawIndex = Math.round(Math.abs(this.scroll.current) / width);
        const centeredIndex = rawIndex % originalLength;
        if (centeredIndex !== this.lastActiveIndex) {
          this.lastActiveIndex = centeredIndex;
          this.onActiveIndexChange(centeredIndex);
        }
      }
    }
  ...
  addEventListeners() {
    ...
    // Scope wheel to container so page scroll still works outside gallery
...
export default function CircularGallery({
  ...
  // Use a ref so callback changes don't recreate the App
  ...
  useEffect(() => {
    ...
    const app = new App(containerRef.current, {
      ...
      onInteractionChange: (isInteracting) => onInteractionChangeRef.current?.(isInteracting),
      posterWidthPx,
      posterHeightPx,
    });
    return () => {
  ...
  }, [items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase, posterWidthPx, posterHeightPx]);
