...
import type { MovieDetailParams } from "@/types/page";
import type { TmdbMovieDetailResponse } from "@/types/api";

async function getMovieDetails(id: string): Promise<TmdbMovieDetailResponse | null> {
  ...
  return (
    <main className="min-h-screen bg-slate-950 pb-20 selection:bg-amber-500/30 selection:text-amber-200">
      ...
      <div className="relative w-full h-[55vh] md:h-[65vh] lg:h-[75vh]">
        ...
        <div className="absolute inset-0 bg-linear-to-t from-slate-950 via-slate-950/80 to-transparent z-0" />
        <div className="absolute inset-0 bg-linear-to-r from-slate-950 via-slate-950/60 to-transparent md:via-slate-950/40 z-0" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-32 md:-mt-48 relative z-10">
        ...
        <div className="flex flex-col md:flex-row gap-8 lg:gap-16">
          {/* Poster Section (Left) */}
          <div className="shrink-0 w-48 mx-auto md:mx-0 md:w-72 lg:w-80">
          ...
          <div className="flex flex-col justify-end pt-4 md:pt-16 pb-2">
            {movie.tagline && <p className="text-amber-400 font-medium tracking-widest text-xs md:text-sm uppercase mb-3 drop-shadow-md">{movie.tagline}</p>}

            <h1 className="text-4xl md:text-5xl lg:text-7xl font-black text-white mb-6 leading-tight tracking-tight drop-shadow-xl">{movie.title}</h1>
            ...
            <div className="flex flex-wrap items-center gap-3 text-sm font-semibold mb-8">
              ...
              <div className="flex items-center gap-1.5 bg-white/5 text-slate-300 py-1.5 px-4 rounded-full border border-white/10 backdrop-blur-md uppercase tracking-wider text-xs">{movie.original_language}</div>
            </div>
            ...
            {/* Synopsis */}
            <div className="mb-8">
              <h3 className="text-xl font-black text-white mb-4 flex items-center gap-2">Synopsis</h3>
              <p className="text-slate-300 leading-relaxed md:text-lg max-w-3xl font-medium opacity-90">{movie.overview || "No overview available for this movie."}</p>
            </div>
          ...
          <section>
            <div className="flex items-center justify-between mb-8 border-b border-white/10 pb-4">
              <div className="flex items-end gap-3">
                ...
                <span className="text-amber-500 font-bold bg-amber-500/10 px-2 py-0.5 rounded text-sm mb-1">{movie.reviews?.results?.length || 0}</span>
              </div>
              ...
              <div className="grid gap-6">
                {topReviews.map(review => (
                  <div
                    ...
                    className="bg-slate-900/50 backdrop-blur-sm rounded-2xl p-6 md:p-8 border border-white/5 hover:border-white/10 transition-colors shadow-lg">
                    <div className="flex items-center justify-between mb-6">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 bg-amber-500 text-amber-950 rounded-full flex items-center justify-center font-black text-xl shadow-md">{review.author[0]?.toUpperCase()}</div>
                        <div>
                          <span className="font-bold text-white block text-lg">{review.author}</span>
                          <span className="text-xs font-medium text-slate-400 uppercase tracking-widest">{new Date(review.created_at).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</span>
                        </div>
                      </div>
                    </div>
                    <p className="text-slate-300 leading-loose text-sm md:text-base opacity-90 line-clamp-6">&ldquo;{review.content}&rdquo;</p>
                  </div>
                ))}
